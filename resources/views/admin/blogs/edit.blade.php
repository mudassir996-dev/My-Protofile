...
<div class="max-w-4xl bg-white border border-slate-200 rounded-3xl shadow-sm p-8">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-sm font-bold text-slate-500 uppercase tracking-widest">Edit Article Form</h2>
    ...
    <form action="{{ route('admin.blogs.update', $blogPost) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center pt-4">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-16 h-12 rounded bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center flex-shrink-0">
                        @if($blogPost->featured_image)
                            <img src="{{ asset('storage/' . $blogPost->featured_image) }}" alt="Preview" class="w-full h-full object-cover">
                        @else
            ...
            <div class="flex items-center h-full pt-4">
                <label for="is_published" class="inline-flex items-center cursor-pointer">
                    ...
                    <span class="ms-3 text-sm font-bold text-slate-700">Published / Visible</span>
                </label>
            </div>
        </div>
