...
<div class="max-w-xl bg-white border border-slate-200 rounded-3xl shadow-sm p-8">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-sm font-bold text-slate-500 uppercase tracking-widest">New Skill Form</h2>
    ...
    <form action="{{ route('admin.skills.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input 
                ...
                value="{{ old('name') }}" 
                required 
                placeholder="e.g. PHP, Laravel, HTML5"
        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                ...
                />
                <span class="text-sm font-bold text-slate-700 w-12 text-right">{{ old('proficiency', 80) }}%</span>
            </div>
        </div>
        ...
        <div>
            ...
            <input 
                ...
                name="icon" 
                value="{{ old('icon') }}" 
                placeholder="e.g. laravel, html5, code"
                class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500"
            />
        </div>
